...
if isinstance(val, dict) and val.get("nonce") and val["nonce"] != st.session_state.last_nonce:
    ...
    # Un handler que revienta NO debe matar el tablero: se captura, se loguea y se avisa.
    try:
        ...
        observabilidad.capturar("app.dispatch", e)
        st.session_state.flash = f"No se pudo completar la acción '{act}': {e}"
